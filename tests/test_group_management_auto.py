import pytest
from unittest.mock import Mock, patch, MagicMock
import json

class TestGroupManagement:
    """Test group management functionality."""
    
    def test_get__groups(self, authenticated_client, mock_ldap_connection):
        """Test GET /groups"""
        with patch('app.get_ldap_connection', return_value=mock_ldap_connection):
            response = authenticated_client.get('/groups', follow_redirects=True)
            assert response.status_code in [200, 302, 401, 403, 429]  # Success, redirect, or auth required
            
    def test_get__api_groups(self, authenticated_client, mock_ldap_connection):
        """Test GET /api/groups"""
        with patch('app.get_ldap_connection', return_value=mock_ldap_connection):
            response = authenticated_client.get('/api/groups', follow_redirects=True)
            assert response.status_code in [200, 302, 401, 403, 429]  # Success, redirect, or auth required
            
