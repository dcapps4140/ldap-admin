...
class TestApiRoutesWithAuth:
    """Test API routes with authentication."""
    
    def test_api_users_get_authenticated(self, authenticated_client, mock_ldap_connection):
        """Test GET /api/users endpoint with authentication."""
        # Mock user data
        mock_user = Mock()
    ...
    def test_api_users_post_authenticated(self, authenticated_client, mock_ldap_connection):
        """Test POST /api/users endpoint with authentication."""
        user_data = {
            'username': 'newuser',
        ...
        with patch('app.get_ldap_connection', return_value=mock_ldap_connection):
            ...
            assert response.status_code in [201, 400, 401, 403, 409, 302]
    
    def test_api_groups_get_authenticated(self, authenticated_client, mock_ldap_connection):
        """Test GET /api/groups endpoint with authentication."""
        # Mock group data
        mock_group = Mock()
